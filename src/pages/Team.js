...
const Team = () => {
    const [activeFilter, setActiveFilter] = useState('executive team');
    const team = {
        executiveTeam: [
            {
    ...
    const SocialLink = ({ href, icon: Icon, label }) => (React.createElement("a", { href: href, target: "_blank", rel: "noopener noreferrer", className: "p-2 rounded-full text-white hover:bg-white/20 transition-colors", "aria-label": label },
        React.createElement(Icon, { size: 20 })));
    const MemberCard = ({ member, type }) => (React.createElement(Card, { className: "group overflow-hidden hover:shadow-xl transition-all duration-300 border-0 bg-gradient-to-r from-emerald-400 to-blue-500" },
        React.createElement(CardHeader, { className: "p-0 relative" },
            React.createElement("img", { src: member.image, alt: member.name, className: `w-full object-cover transition-transform duration-300 group-hover:scale-105 ${type === 'executive team' ? 'h-72' : 'h-48'}` }),
        ...
        React.createElement(CardContent, { className: "p-6 text-white" },
            React.createElement("h3", { className: "text-xl font-bold mb-2" }, member.name),
            React.createElement("p", { className: "text-emerald-200 font-semibold mb-3" }, member.role),
            React.createElement("p", { className: "mb-6" }, member.bio),
    ...
    const AdvisorCard = ({ advisor }) => (React.createElement(Card, { className: "group hover:shadow-xl transition-all duration-300 border-0 bg-gradient-to-r from-teal-400 to-blue-500" },
        React.createElement(CardContent, { className: "p-8 flex gap-6 items-start text-white" },
            React.createElement("img", { src: advisor.image, alt: advisor.name, className: "w-24 h-24 rounded-full object-cover ring-2 ring-offset-2 ring-emerald-200 transition-transform duration-300 group-hover:scale-105" }),
            React.createElement("div", null,
                React.createElement("h3", { className: "text-xl font-bold mb-2" }, advisor.name),
                React.createElement("p", { className: "text-emerald-200 font-semibold mb-3" }, advisor.role),
                React.createElement("p", { className: "mb-4" }, advisor.bio),
                advisor.linkedin && (React.createElement("a", { href: advisor.linkedin, target: "_blank", rel: "noopener noreferrer", className: "inline-flex items-center gap-2 text-emerald-200 hover:text-white" },
    ...
    return (React.createElement("div", { className: "min-h-screen bg-black" },
        React.createElement("header", { className: "py-24 text-center relative" },
            React.createElement("div", { className: "container mx-auto px-4" },
                React.createElement("h1", { className: "text-5xl font-bold mb-6 text-white" }, "Meet Our Team"),
                React.createElement("p", { className: "text-xl max-w-2xl mx-auto leading-relaxed text-gray-300" }, "A diverse group of professionals dedicated to fostering excellence in business and science"))),
        React.createElement("div", { className: "container mx-auto px-4 mb-12" },
            React.createElement("div", { className: "flex justify-center gap-4 flex-wrap" }, ['executive team', 'managerial team', 'advisory', 'society members'].map((filter) => (React.createElement(Button, { key: filter, variant: activeFilter === filter ? "default" : "outline", onClick: () => setActiveFilter(filter), className: `capitalize ${activeFilter === filter
                    ? 'bg-gradient-to-r from-emerald-400 to-blue-500 text-white border-none'
                    : 'text-white border-emerald-400 hover:bg-emerald-400/20'}` }, filter))))),
        React.createElement("div", { className: "container mx-auto px-4 pb-24" },
            activeFilter === 'executive team' && (React.createElement("section", { className: "mb-20" },
                React.createElement("h2", { className: "text-3xl font-bold text-center mb-12 text-white" }, "Executive Team"),
                React.createElement("div", { className: "grid md:grid-cols-2 gap-8" }, team.executiveTeam.map((member) => (React.createElement(MemberCard, { key: member.name, member: member, type: "executive team" })))))),
            activeFilter === 'managerial team' && (React.createElement("section", { className: "mb-20" },
                React.createElement("h2", { className: "text-3xl font-bold text-center mb-12 text-white" }, "Managerial Team"),
                React.createElement("div", { className: "grid md:grid-cols-2 gap-8" }, team.managerialTeam.map((member) => (React.createElement(MemberCard, { key: member.name, member: member, type: "managerial team" })))))),
            activeFilter === 'advisory' && (React.createElement("section", { className: "mb-20" },
                React.createElement("h2", { className: "text-3xl font-bold text-center mb-12 text-white" }, "Advisory"),
                React.createElement("div", { className: "grid md:grid-cols-2 gap-8" }, team.advisory.map((advisor) => (React.createElement(AdvisorCard, { key: advisor.name, advisor: advisor })))))),
            activeFilter === 'society members' && (React.createElement("section", { className: "mb-20" },
                React.createElement("h2", { className: "text-3xl font-bold text-center mb-12 text-white" }, "Society Members"),
                React.createElement("div", { className: "grid md:grid-cols-2 gap-8" }, team.societyMembers.map((member) => (React.createElement(MemberCard, { key: member.name, member: member, type: "society members" })))))))));
};
export default Team;
